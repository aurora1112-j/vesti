...
接下来的一至两个月，我们的目标是将零散的对话转化为可视化的个人知识库。以下是预览效果：

![Vesti Dashboard Preview](assets/dashboard-effect.png)
*未来版本规划：集成活跃度热力图与多模型分布统计*

#### 1. 完善全平台支持
> 注意看上图的 **Platform Distribution** 区域

我们将添加 **Gemini** 和 **DeepSeek** 的后端捕获能力（如效果图所示），并探索支持更多 AI 工具（Perplexity, Poe）。这意味着无论你在哪里对话，Vesti 都能将其汇总到同一个 Activity Heatmap 中。

#### 2. 引入向量化技术
将所有对话内容转换为语义向量存储在本地。这将支持真正的语义搜索——你搜索 "产品设计" 时，系统不仅匹配关键词，还能关联到用户体验、交互逻辑等相关对话。

#### 3. 实现跨对话主题聚类
通过向量相似度计算，系统将自动生成类似 **Activity Heatmap** 的思维图谱。识别你在不同时间反复讨论的核心议题，让你直观看到思维如何在不同话题间流动。

#### 4. 开发可选的云端同步
采用端到端加密方案，确保数据隐私。用户可选择是否开启同步，默认维持 **Local-First**（本地优先）策略。

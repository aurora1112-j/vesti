...
## 1.1 Frozen Text Gate Status

本轮 Week 2 的四个 operator text samples 不再作为开放调查输入使用，而是作为 shipped code 的 frozen acceptance gate：
- `SEARCH_CITATION_001`
- `CLAUDE_ARTIFACT_001`
- `CLAUDE_TITLE_001`
- `TABLE_FIDELITY_001`
...
- frozen as acceptance input
- any remaining boundary is deferred, not missing
...
- `CLAUDE_TITLE_001` 已在 shipped Claude parser 中闭环
- title precedence 已改为 app shell first，sidebar fallback second，generic page/body fallback last
- 该 case 现在属于 frozen acceptance input，而不是开放 root-cause note

Remaining deferred boundary:
- 其它平台的 app-shell title 词典仍可继续扩展，但这不影响 Claude 当前 contract 已闭环
...
Current shipped status:
- `TABLE_FIDELITY_001` 对应的 shipped baseline 已落到 `semantic_ast_v2`
- cell-level inline-rich children、KaTeX `annotation` recovery、alignment extraction 已进入现有代码
- 该 case 现在是 frozen acceptance input，用来约束 canonical table/math/code fidelity

Remaining deferred boundary:
- pseudo-table 与更深的跨平台 parser layering 仍可继续硬化
- 这些是下一 slice 的结构增强，不代表当前 shipped baseline 缺失
...
Current shipped status:
- `SEARCH_CITATION_001` 已在 shipped implementation 中闭环
- citation pill 已从 clone body 中剥离，label 收口到第一条可见行，`utm_*` 已在持久化前剥离
- citations 以 sidecar 形式持久化和导出，不再允许回流到 `bodyText`

Remaining deferred boundary:
- 更多平台的 citation family 仍可扩展 sourceType 词典
- 这属于 coverage 扩边，不属于当前 frozen case 的 contract 缺口
...
Current shipped status:
- `CLAUDE_ARTIFACT_001` 对应的 standalone artifact sidecar 已在 shipped code 中闭环
- `plainText`、`normalizedHtmlSnapshot`、safe `markdownSnapshot`、`renderDimensions` 已进入当前 contract
- artifact content 继续保持在 sidecar，不进入正文主干，也不是 body-tail reconstruction 来源

Remaining deferred boundary:
- artifact replay / iframe execution / interactive preview 仍明确 deferred
- richer artifact extraction 仍可继续扩展，但不需要改当前 schema 才能认定 shipped contract 成立
